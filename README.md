...
<h1 align="center">Deep AI Worker</h1>

<p align="center">
  <strong>Let Intelligence Work</strong>
</p>

<p align="center">
  <a href="#features">Features</a> •
  <a href="#why-deep-ai-worker">Why Deep AI Worker</a> •
...
**Deep AI Worker** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience with a brand centered on operational intelligence.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, Deep AI Worker provides the interface you need to put intelligence to work.

Deep AI Worker comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings. You can also fine-tune advanced configurations via **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. Deep AI Worker was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | Deep AI Worker Solution |
|-----------|----------------|
...
Deep AI Worker is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
Chat attachments and generated files can now preview directly inside the conversation with inline support for common documents, PDFs, images, code, spreadsheets, and presentations. Word previews use an interactive DOCX/HTML renderer with outline navigation, while Excel and CSV previews use selectable sheet tables with sheet tabs, sticky first rows, and lazy-loaded row/column windows for larger sheets instead of static page images. When you expand a preview, it opens in its own larger detachable window so you can drag it onto another screen while keeping the main chat visible, but it no longer defaults to filling the entire screen. Presentation previews also use a roomier file-size cap than the generic preview fallback, render visually as continuous vertically scrollable slide pages when slide extraction succeeds, and keep only a temporary local preview cache so the app does not treat slide images as exported user files. Deep AI Worker now uses a local LibreOffice/Impress runtime for presentation-style previews when available; if LibreOffice is missing, the preview panel prompts the user to download the runtime on demand and automatically reloads the preview after installation instead of bundling LibreOffice inside the installer.
...
Each channel now supports multiple accounts, per-account agent binding, and switching the channel default account directly from the Channels page. WeChat shows linked WeChat accounts without exposing internal bot IDs, and each WeChat account has its own Role binding.
Deep AI Worker now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. Deep AI Worker adapts to your preferences automatically.
...
When you launch Deep AI Worker for the first time, the **Setup Wizard** will guide you through:
...
> Note for Moonshot (Kimi): Deep AI Worker keeps Kimi web search enabled by default.  
> When Moonshot is configured, Deep AI Worker also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

Deep AI Worker includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, Deep AI Worker falls back to `Proxy Server`.
...
Deep AI Worker employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
└──────────────────────────────┬──────────────────────────────────┘
                               ...
                               │ (IPC default, optional WS/HTTP diagnostics)
                               ▼
┌─────────────────────────────────────────────────────────────────┐
...
- **Main-Process Transport Ownership**: Electron Main owns transport selection; runtime RPC defaults to IPC, while WS/HTTP remain available for diagnostics and compatibility work
...
- Deep AI Worker is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
LibreOffice is intentionally not bundled in installers. When a presentation preview needs LibreOffice and no local runtime is found, Deep AI Worker asks the user whether to download it, shows download progress, stores the runtime under the user data directory, and reloads the preview automatically after installation.
...
- uses a temporary Deep AI Worker `userData` directory
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make Deep AI Worker better.
...
Deep AI Worker is built on the shoulders of excellent open-source projects:
...
### Deep AI Worker Partner Program 🚀

We're launching the Deep AI Worker Partner Program and looking for partners who can help introduce Deep AI Worker to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the Deep AI Worker team provides full technical support, customization, and integration.
...
Deep AI Worker is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
