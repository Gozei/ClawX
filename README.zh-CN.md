...
聊天中的附件和生成文件现在默认仍可在会话页右侧分栏预览，常见文档、PDF、图片、代码、表格和演示文稿都支持内嵌查看。Word 预览现在改为更高保真的 DOCX 渲染方式，主题字体、标题颜色和表格样式会尽量保留，不再只剩下扁平化的语义文本。点击放大后，会打开一个较大的独立预览窗口，方便拖到副屏或软件外单独查看，但不会默认整屏，同时保留下载等操作。演示文稿预览还单独放宽了文件体积阈值；当可视化渲染成功时，会按连续纵向翻页的方式展示更接近真实幻灯片的页面内容，并把渲染结果缓存到本地，重复打开同一个 `.pptx` 会明显更快。在 Windows 上，Deep AI Worker 现在会优先调用本机安装的 PowerPoint 把 PPT 导出为高保真预览图片；只有在该路径不可用时，才会回退到 HTML 幻灯片渲染。
