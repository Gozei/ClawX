...
Marketplace 来源现在通过内部的 `skill-sources.json` 管理。ClawX 默认会初始化两个来源（`ClawHub` 与当前环境变量导入的 `deepaiworker` 来源），并为每个来源分配独立的 ClawHub workdir（位于 `~/.openclaw/skill-sources/<source-id>`）；Skills 页面支持“全部来源搜索”和“按单一来源搜索”。
