...
聊天中的附件和生成文件现在默认仍可在会话页右侧分栏预览，常见文档、PDF、图片、代码、表格和演示文稿都支持内嵌查看。Word 仍保留高保真的 DOCX 兜底渲染，同时 Word、Excel、CSV 现在会优先走 LibreOffice 生成分页预览，长文本和原生分页会以连续可滚动页面展示。点击放大后，会打开一个较大的独立预览窗口，方便拖到副屏或软件外单独查看，但不会默认整屏，同时保留下载等操作。演示文稿预览还单独放宽了文件体积阈值；当可视化渲染成功时，会按连续纵向翻页的方式展示更接近真实幻灯片的页面内容，并只保留临时本地预览缓存，不会把这些页图当作用户导出的图片长期保存。Deep AI Worker 现在会使用本地 LibreOffice / Impress 运行时处理 Office 类预览；如果用户机器未检测到 LibreOffice，会在预览时弹窗提示用户按需下载，下载完成后自动重新加载预览，不再把 LibreOffice 内置进安装包。
...
LibreOffice 不会随安装包内置。Office 预览需要 LibreOffice 且本机未检测到运行时时，Deep AI Worker 会提示用户是否下载，展示下载进度，并把运行时安装到用户数据目录；安装完成后会自动重新加载预览窗口。
