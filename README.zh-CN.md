...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-deep-ai-worker">为什么选择 Deep AI Worker</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**Deep AI Worker** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观、强调业务运行能力的桌面体验。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，Deep AI Worker 都能提供高效易用的图形界面，帮助你真正把智能能力投入工作。

Deep AI Worker 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行更细致的高级配置。
...
## 为什么选择 Deep AI Worker

构建 AI 智能体不应该需要精通命令行。Deep AI Worker 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**

| 痛点 | Deep AI Worker 解决方案 |
|------|----------------|
...
Deep AI Worker 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，Deep AI Worker 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
每个 Agent 还可以单独覆盖自己的 `provider/model` 运行时设置；未覆盖的 Agent 会继续继承全局默认模型。
现在也支持从侧边栏归档会话，并在 **设置 → 会话归档** 中统一查看和恢复，归档不会删除消息、文件或会话内容。
聊天中的附件和生成文件现在默认仍可在会话页右侧分栏预览，常见文档、PDF、图片、代码、表格和演示文稿都支持内嵌查看。Word 使用可交互的 DOCX/HTML 渲染并保留目录定位，Excel 和 CSV 使用可选择的 sheet 表格、sheet 标签页、固定首行，并在大表时滚动到边缘自动加载行/列窗口，而不是静态页图。点击放大后，会打开一个较大的独立预览窗口，方便拖到副屏或软件外单独查看，但不会默认整屏，同时保留下载等操作。演示文稿预览还单独放宽了文件体积阈值；当可视化渲染成功时，会按连续纵向翻页的方式展示更接近真实幻灯片的页面内容，并只保留临时本地预览缓存，不会把这些页图当作用户导出的图片长期保存。Deep AI Worker 现在会使用本地 LibreOffice / Impress 运行时处理演示文稿类预览；如果用户机器未检测到 LibreOffice，会在预览时弹窗提示用户按需下载，下载完成后自动重新加载预览，不再把 LibreOffice 内置进安装包。
在 **设置 → 通用** 中，你现在也可以为聊天附件选择独立的上传目录；ClawX 会按 agent / session 自动创建 `uploads` 子目录来管理这些文件，而不是把所有附件都混在同一个共享目录里。你也可以单独设置模型输出目录，ClawX 会把识别到的模型产出复制到按 agent / session 分组的 `outputs` 子目录中，方便按会话整理文件。

### 📡 多通道管理
同时配置和监控多个 AI 通道。每个通道独立运行，允许你为不同任务运行专门的智能体。
现在每个通道支持多个账号，并可在 Channels 页面直接完成账号绑定到 Agent 与默认账号切换。微信通道会显示已连接的微信账号，不暴露内部 bot ID；每个微信账号都有独立的 Role 绑定。
Deep AI Worker 现在还内置了腾讯官方个人微信通道插件，可直接在 Channels 页面通过内置二维码流程完成微信连接。

### ⏰ 定时任务自动化
调度 AI 任务自动执行。定义触发器、设置时间间隔，让 AI 智能体 7×24 小时不间断工作。
现在定时任务工作台包含网关状态、分页任务管理、运行历史、详情抽屉和高级任务配置。你可以配置 `at`、`every`、cron 调度及时区/错峰选项，绑定 Agent 与会话目标，选择 payload 执行方式，查看运行错误和投递状态，并继续在任务表单中直接配置外部投递的发送账号与接收目标。

### 🌙 梦境模式
在 **设置 → 通用** 中开启 **梦境模式** 后，客户端会把 OpenClaw `memory-core` 的 dreaming 配置同步到核心运行时，并显示梦境工作区、梦境日志、状态与维护操作。关闭后，梦境相关入口会隐藏，OpenClaw dreaming 配置也会被写为关闭。梦境工作区还提供单一的 **记忆沉淀速度** 配置，用来控制信息进入 `MEMORY.md` 的难易快慢，底层阈值和计划由系统统一映射。

### 🧩 可扩展技能系统
通过预构建的技能扩展 AI 智能体的能力。在集成的技能面板中浏览、安装和管理技能——无需包管理器。
Deep AI Worker 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。
Skills 页面可展示来自多个 OpenClaw 来源的技能（托管目录、workspace、额外技能目录），并显示每个技能的实际路径，便于直接打开真实安装位置。
技能详情页现在会聚合运行时诊断、注入到 `skills.entries` 的配置值，以及常见的本地技能文件（如 `config.json` / `.env`），让用户可以在一个界面里查看并补齐必填或选填配置。
Marketplace 来源现在通过内部的 `skill-sources.json` 管理。ClawX 默认会初始化两个来源（`ClawHub` 与当前环境变量导入的 `DeepSkillHub` 来源，id 为 `deepaiworker`），并为每个来源分配独立的 ClawHub workdir（位于 `~/.openclaw/skill-sources/<source-id>`）；Skills 页面支持“全部来源搜索”和“按单一来源搜索”。
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。Deep AI Worker 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 Deep AI Worker 在系统登录后自动启动。

### 日志与审计面板
在 **设置 → 网关** 中，Deep AI Worker 内置了 **日志与审计** 面板，支持以本地时间查看运行日志和审计留痕，并按关键词、文件、时间范围进行筛选和导出。
同一个面板里还可以直接调整应用日志保留天数、审计日志保留天数、单文件大小上限、日志级别和审计模式，不需要再进入开发者模式。
...
首次启动 Deep AI Worker 时，**设置向导** 将引导你完成以下步骤：
...
> Moonshot（Kimi）说明：Deep AI Worker 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，Deep AI Worker 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

Deep AI Worker 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类通道的联网请求。
...
- 如果启用了 Telegram，Deep AI Worker 还会把代理同步到 OpenClaw 的 Telegram 通道配置中。
- 当 Deep AI Worker 代理处于关闭状态时，Gateway 的常规重启会保留已有的 Telegram 通道代理配置。
- 如果你要明确清空 OpenClaw 中的 Telegram 代理，请在关闭代理后点一次“保存代理设置”。
- 设置中心里的 **OpenClaw Console** 入口属于开发者工具，只有开启 **开发者模式** 后才会显示。
...
Deep AI Worker 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
- **主进程掌控传输策略**：运行时 RPC 默认走 IPC，WS/HTTP 由主进程按诊断或兼容需要启用，稳定性优先
...
- Deep AI Worker 基于 Electron，**单个应用实例出现多个系统进程是正常现象**（main/renderer/zygote/utility）。
...
### 📊 自动化监控
设置定时智能体来监控新闻动态、追踪价格变动或监听特定事件。结果将推送到你偏好的通知通道。
...
LibreOffice 不会随安装包内置。演示文稿预览需要 LibreOffice 且本机未检测到运行时时，Deep AI Worker 会提示用户是否下载，展示下载进度，并把运行时安装到用户数据目录；安装完成后会自动重新加载预览窗口。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 Deep AI Worker 变得更好。
...
### Deep AI Worker 合作伙伴计划 🚀

我们正在启动 Deep AI Worker 合作伙伴计划，寻找能够帮助我们将 Deep AI Worker 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，Deep AI Worker 团队则提供完整的技术支持、定制开发与集成服务。
...
Deep AI Worker 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
