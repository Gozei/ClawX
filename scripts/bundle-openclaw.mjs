...
function patchHashedDistFile(prefix, replacer) {
  ...
  for (const fileName of fs.readdirSync(distDir)) {
    ...
    if (patchFileText(path.join(distDir, fileName), replacer)) {
      count += 1;
    }
  }
  return count;
...
function patchOpenClawPrompts() {
  let patchedFiles = 0;

  patchedFiles += patchHashedDistFile('reply-', (text) =>
    text.replace(SESSION_RESET_PROMPT_EN, SESSION_RESET_PROMPT_ZH)
  );

  patchedFiles += patchHashedDistFile('current-time-', (text) =>
    text.replace(
  ...
  patchedFiles += patchHashedDistFile('restart-sentinel-', (text) =>
    text
      .replace(
