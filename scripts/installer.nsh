; Deep AI Worker Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting Deep AI Worker runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers the Electron app executable (multiple processes), openclaw-gateway.exe,
  ...
  ; "Can't modify app files" retry loop -> blocking close dialogs.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking close dialog.
...
; Retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with the app's
