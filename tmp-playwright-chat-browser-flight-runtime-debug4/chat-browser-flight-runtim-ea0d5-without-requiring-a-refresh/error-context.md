...
```ts
  ...
  265 |       console.log('chat-history-apply-debug-after-runtime:', await page.evaluate(() => (globalThis as typeof globalThis & { __chatHistoryApplyDebug?: unknown }).__chatHistoryApplyDebug));
  266 | 
> 267 |       await expect(page.getByText('已帮你整理明天深圳到南京的机票信息：', { exact: true })).toBeVisible({ timeout: 60_000 });
      |                                                                           ^ Error: expect(locator).toBeVisible() failed
  268 |       await expect(page.getByText('东方航空', { exact: true })).toBeVisible({ timeout: 60_000 });
  269 |       await expect(page.getByText('MU2878', { exact: true })).toBeVisible({ timeout: 60_000 });
  270 |       await expect(page.getByText('￥620 起', { exact: true })).toBeVisible({ timeout: 60_000 });
  271 | 
  272 |       await expect(page.getByTestId(`sidebar-session-running-indicator-${SESSION_KEY}`)).toHaveCount(0, { timeout: 20_000 });
  273 |       await expect.poll(async () => {
  274 |         return await sendButton.evaluate((node) => !!node.querySelector('svg.lucide-send-horizontal'));
  275 |       }, { timeout: 20_000 }).toBe(true);
  276 | 
  277 |       const sendButtonTitle = await sendButton.getAttribute('title');
  278 |       expect(['Send', '发送']).toContain(sendButtonTitle);
  279 | 
  280 |       await page.waitForTimeout(1_500);
  281 |       await expect(page.getByTestId(`sidebar-session-running-indicator-${SESSION_KEY}`)).toHaveCount(0);
  282 |       await expect.poll(async () => {
  283 |         return await sendButton.evaluate((node) => !!node.querySelector('svg.lucide-send-horizontal'));
  284 |       }).toBe(true);
  285 |     } finally {
  286 |       await closeElectronApp(app);
  287 |     }
  288 |   });
  289 | });
  290 | 
