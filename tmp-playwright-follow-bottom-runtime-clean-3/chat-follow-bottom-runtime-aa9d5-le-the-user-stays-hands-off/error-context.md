...
Locator: getByText('阶段性整理：下一段我会继续补白天时段和筛选建议。')
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for getByText('阶段性整理：下一段我会继续补白天时段和筛选建议。')
...
```ts
  ...
  293 | 
> 294 |       await expect(page.getByText('阶段性整理：下一段我会继续补白天时段和筛选建议。')).toBeVisible({ timeout: 20_000 });
      |                                                                ^ Error: expect(locator).toBeVisible() failed
  ...
  300 | 
  301 |       await expect(page.getByText('补充说明七：如果你只接受直飞，我也可以只保留直飞航班。')).toBeVisible({ timeout: 20_000 });
