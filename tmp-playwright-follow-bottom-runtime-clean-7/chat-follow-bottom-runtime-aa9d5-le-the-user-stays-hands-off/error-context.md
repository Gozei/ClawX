...
```ts
  ...
  304 | 
  305 |       await expect(page.getByText('补充说明七：如果你只接受直飞，我也可以只保留直飞航班。')).toBeVisible({ timeout: 20_000 });
> 306 |       await expect.poll(async () => {
      |       ^ Error: expect(received).toBe(expected) // Object.is equality
  307 |         const metrics = await measureScrollMetrics(page);
  308 |         return metrics.scrollTop > metricsAfterSecondChunk.scrollTop
  309 |           && (metrics.distanceFromBottom ?? 999) <= 24;
